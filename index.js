...
class Chart {
  /* создание svg элемента  ( имя тега )*/
...
class LineChart extends Chart {
  ...
  createTicks() {
    ...
    // Сколько у нас их будет
    const ticksCount = this.maxChartHeight / heightPerTick;
    // Разница между настоящими значениями "Y" для каждое пометки
  ...
  create() {
    ...
    this.data.forEach((el, i) => {
      const x = currentX;
      const y = this.maxHeight - el.y * this.zoom - yShift - this.topYPadding;
      console.log(y);
      ...
      //      $legendXText.append(el.x); /* можно так  */
      $legendXText.textContent = el.x; /* можно так  */
